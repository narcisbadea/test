import { Component, OnInit, ViewChild } from '@angular/core';
import { MatDialog } from '@angular/material/dialog';
import { MatPaginator } from '@angular/material/paginator';
import { MatSort } from '@angular/material/sort';
import { MatTableDataSource } from '@angular/material/table';
import { ItemResponseDto, ItemResponseForClientDto } from 'src/app/api/models';
import { AdminItemsService, ClientItemService } from 'src/app/api/services';
import { SnackbarHelperService } from 'src/app/_helpers/snackbar-helper.service';
import { BidComponent } from '../dashboard/bid/bid.component';
import { ApproveComponent } from '../item-approve/approve/approve.component';
import { saveAs } from 'file-saver';

@Component({
  selector: 'app-my-items',
  templateUrl: './my-items.component.html',
  styleUrls: ['./my-items.component.scss']
})
export class MyItemsComponent implements OnInit {
  @ViewChild(MatPaginator, { static: true }) paginator!: MatPaginator;
  @ViewChild(MatSort, { static: true }) sort!: MatSort;

  // ADD YOUR MODEL HERE
  // TODO: Add model
  public auctions: ItemResponseDto[] = [];

  // NAME THE COLUMNS YOU WANT TO LIST IN THE TABLE
  // PLEASE CHECK ALSO THE HTML matColumnDef AND element.columnX
  // TODO: Change column names here and in html
  public displayedColumns = ['name', 'desc', 'endTime', 'price', 'state', 'report'];

  public items = new MatTableDataSource(this.auctions);

  public length: number = 0;

  constructor(private adminItem: AdminItemsService, private dialog: MatDialog, private snackbarHelper: SnackbarHelperService, private itemService: ClientItemService) {}

  ngOnInit(): void {
    this.initData();
    this.itemService.apiClientItemMyItemaNumberGet$Json().subscribe((result: number) =>{
      this.paginator.length = result;
      console.log(length);
    })
  }

  // GET DATA FROM SERVICE
  private initData(): void {
    
    this.itemService.apiClientItemMyItemsPageGet$Json({nr:1}).subscribe((itemsResponse: ItemResponseDto[])=>{
      this.auctions = itemsResponse;
      console.log(this.auctions);
      this.items = new MatTableDataSource(this.auctions);
      //this.items.paginator = this.paginator;
      this.items.sort = this.sort;
      itemsResponse.forEach((item)=>{
        this.itemService.apiClientItemItemStateGet$Json({itemId: item.id}).subscribe((res: string)=>{
          item.state = res;
        })
      })
    });
    
  }

  // DEFINE FILTER FOR SEARCH
  public applyFilter(event: Event): void {
    const filterValue = (event.target as HTMLInputElement).value;
    this.items.filter = filterValue.trim().toLowerCase();

    if (this.items.paginator) {
      this.items.paginator.firstPage();
    }
  }
  public create() {
    const dialogRef = this.dialog.open(BidComponent, {
      width: '500px'
    });
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        this.initData();
      }
    });
  }

  public report(itemID:number) {
    this.adminItem.apiAdminItemsIdGet$Json({id: itemID}).subscribe((data)=>{
      saveAs(data, `report.xlsx`);
    },
     (e: any) => {
      this.snackbarHelper.openSnackbar('Error generating report', 'bg-danger');
    })
  }

  nextpage(){
    this.itemService.apiClientItemMyItemsPageGet$Json({nr:(this.paginator.pageIndex + 1)}).subscribe((itemsResponse: ItemResponseDto[])=>{
      this.auctions = itemsResponse;
      
      this.items = new MatTableDataSource(this.auctions);
      this.items.sort = this.sort;
      itemsResponse.forEach((item)=>{
        this.itemService.apiClientItemItemStateGet$Json({itemId: item.id}).subscribe((res: string)=>{
          item.state = res;
        })
      })
    })
  }
}
